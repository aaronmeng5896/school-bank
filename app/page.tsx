      ...
      <div className="content-grid">
        ...
        <aside className="detail-panel">
          ...
          <div className="detail-body">
            {activeTab==="核心数据" && <><p className="summary">{selected.summary}</p><div className="metric-grid"><article><span>学生总数</span><strong>{selected.students}</strong><small>{selected.verified?"当前公开学年":"等待学校官网同步"}</small></article><article><span>寄宿比例</span><strong>{selected.boardingRate}</strong><small>{selected.verified?"寄宿生占学生总数":"等待学校官网同步"}</small></article><article><span>国际生比例</span><strong>{selected.internationalRate}</strong><small>{selected.verified&&selected.internationalRate!=="待核验"?"学校官方口径":"等待学校官网同步"}</small></article><article><span>课程数量</span><strong>{selected.courseCount}</strong><small>{selected.verified?"学校公开课程范围":"等待学校官网同步"}</small></article><article><span>校园面积</span><strong>{selected.campusArea}</strong><small>{selected.verified?"学校官网公布":"等待学校官网同步"}</small></article><article><span>寄宿学费</span><strong>{selected.tuition}</strong><small>{selected.verified?"当前公开学年":"等待学校官网同步"}</small></article><article><span>平均班级</span><strong>{selected.classSize}</strong><small>{selected.verified?"名学生":"等待学校官网同步"}</small></article><article><span>开设年级</span><strong>{selected.grades}</strong><small>{selected.verified?"官方项目范围":"目录参考字段"}</small></article></div><section className="advisor-note"><div className="note-icon">i</div><div><strong>顾问提示</strong><p>FindingSchool 仅用于建立学校范围索引，不作为官方事实字段来源。发送给家庭前，请等待学校官网或政府数据库完成核验。</p></div></section></>}
            {activeTab==="申请信息" && (selected.application ? <div className="application-section"><div className="application-head"><div><span>官网核验 · {selected.application.verifiedOn}</span><h3>{selected.application.cycle} 申请信息</h3></div><a href={selected.application.sourceUrl} target="_blank" rel="noreferrer">查看官网申请页 ↗</a></div><div className="application-meta"><article><span>申请截止</span><strong>{selected.application.deadline}</strong></article><article><span>申请平台</span><strong>{selected.application.platform}</strong></article></div><div className="application-block international"><span className="application-number">INTL</span><div><h4>国际生特别要求</h4><p>{selected.application.international}</p></div></div><div className="application-block"><span className="application-number">TEST</span><div><h4>标准化考试</h4><p>{selected.application.testing}</p></div></div><div className="application-block"><span className="application-number">INTV</span><div><h4>面试形式与要求</h4><p>{selected.application.interview}</p></div></div><div className="materials"><h4>申请材料清单</h4><ul>{selected.application.materials.map(item=><li key={item}>{item}</li>)}</ul></div><p className="application-disclaimer">信息为官网内容的中文概括；申请政策可能随年度调整，正式递交前请再次核对学校官网。</p></div> : <div className="application-pending"><span>↻</span><h3>申请信息等待官网核验</h3><p>该校已建立目录档案，但国际生材料、标准化考试、面试和截止日期尚未从学校 Admission 页面完成逐项核验。</p><a href={selected.sourceUrl} target="_blank" rel="noreferrer">前往当前来源页 ↗</a></div>)}
            {activeTab==="学术课程" && (selected.academics ? <div className="academics-section"><div className="academics-head"><div><span>官方课程目录 · {selected.academics.catalogYear}</span><h3>学术课程与课程介绍</h3></div><a href={selected.academics.sourceUrl} target="_blank" rel="noreferrer">打开完整课程目录 ↗</a></div><div className="academics-overview"><div><strong>{selected.academics.totalCourses}</strong><span>官网课程规模</span></div><p>{selected.academics.overview}</p></div><div className="department-list">{selected.academics.departments.map((department,index)=><details key={department.name} open={index===0}><summary><div><span>{String(index+1).padStart(2,"0")}</span><strong>{department.name}</strong></div><b>{department.courses.length} 门代表课程⌄</b></summary><p className="department-description">{department.description}</p><div className="course-list">{department.courses.map(course=><article key={course.code}><div className="course-title"><span>{course.code}</span><h4>{course.title}</h4></div><i>{course.level}</i><p>{course.description}</p></article>)}</div></details>)}</div><p className="academic-disclaimer">当前展示顾问高频代表课程；完整 {selected.academics.totalCourses} 课程、先修条件、开课学期及毕业要求请以官方目录为准。官网核验日期：{selected.academics.verifiedOn}。</p></div> : <div className="application-pending"><span>↻</span><h3>课程目录等待官网同步</h3><p>该校的 Curriculum / Course Catalog 尚未完成逐项提取。同步后将按学科展示课程代码、简介、层级、先修要求和官方目录链接。</p><a href={selected.sourceUrl} target="_blank" rel="noreferrer">前往当前来源页 ↗</a></div>)}
